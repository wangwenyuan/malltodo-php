...
class UploadTDController extends TDCONTROLLER
{
    ...
    public function index()
    {
        ...
        if (! $info) {
            ...
            $ret = array();
            $ret["error"] = 0;
            $ret["url"] = TD_URL . str_replace('./', '/', TDConfig::$upload["rootPath"]) . $info["savepath"] . $info["savename"];
    ...
    public function base64()
    {
        ...
        if (TD_IS_POST) {
            ...
            $data[FILE::$agent_id] = $agent_id;
            $data[FILE::$merchant_id] = $merchant_id;
            $data[FILE::$url] = TD_URL . str_replace('./', '/', TDConfig::$upload["rootPath"]) . $save_dir . "/" . $save_file;
            ...
            $ret = array();
            $ret["error"] = 0;
            $ret["url"] = TD_URL . str_replace('./', '/', TDConfig::$upload["rootPath"]) . $save_dir . "/" . $save_file;
            echo json_encode($ret, JSON_UNESCAPED_UNICODE);
        }
    }
}