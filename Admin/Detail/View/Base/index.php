  ...
  <table width="100%" cellpadding="0" cellspacing="0" class="main_table">
    ...
    if (count($list) == 0) {
        ...
        for ($i = 0; $i < count($list); $i = $i + 1) {
            ...
            echo "<td>" + $jsonObject[DETAIL::$category_name] + "</td>";
            echo "<td>" + $jsonObject[DETAIL::$sort] + "</td>";
            $edit_url = TDU(TD_MODULE_NAME . "/" . TD_CONTROLLER_NAME . "/edit", array(
                "id" => $jsonObject["id"]
            ));
            $del_url = TDU(TD_MODULE_NAME . "/" . TD_CONTROLLER_NAME . "/del");
            echo "<td><a href=\"javascript:malltodoJs.sub_window('编辑', '" . $edit_url . "')\">修改</a> <a href=\"javascript:malltodoJs.del('" . $del_url . "','" . $jsonObject[$i] . "')\">删除</a></td>";
            echo "</tr>";
        }
    }
    ?>
